...
### Toggleable Overlay
The Embedded Photography filter is available in the primary and preserved Preview interfaces. In the primary `2.0.0` interface it is explicitly labeled an experimental metadata estimate.

> **Current-data provenance:** the committed scores were generated by the repository's mock heuristic mode. They are useful for exercising the facet pipeline but are not model inspection of, or evidence from, a physical edition. `Mock:` reasoning must remain visibly labeled until the dataset is rescored with accurate provider/model provenance.
